// TCS230 or TCS3200 pins wiring to ESP8266
#define S0 D2
#define S1 D3
#define S2 D4
#define S3 D5
...
void setup() {
  ...
  Serial.begin(115200);  // Use Serial1 for ESP8266

  delay(500);  // Allow some time for the serial monitor to open
  Serial.println("ESP8266 Color Detection");
}
...
void loop() {
  ...
  greenColor = map(greenFrequency, 80, 180, 255, 0);  // Adjusted range for green color

  // Reading the BLUE (B) frequency
  digitalWrite(S2, LOW);
  digitalWrite(S3, HIGH);
  blueFrequency = pulseIn(sensorOut, LOW);
  blueColor = map(blueFrequency, 20, 70, 255, 0);  // Adjusted range for blue color
